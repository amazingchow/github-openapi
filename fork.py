...
def fork(session, owner, token, repo):
    _Headers = HEADERS
    _Headers["Authorization"] = "token {0}".format(token)

    resp = session.post(
        url="https://api.github.com/repos/{0}/{1}/forks".format(owner, repo), headers=_Headers)
    if resp.status_code == 202:
        CustomLogger.info("fork repo <{0}/{1}>".format(owner, repo))
    elif resp.status_code == 429:
        raise RateLimitException("api response: 429", 5)    
    else:
        CustomLogger.error("failed to fork repo <{0}/{1}>, status code: {2}".format(owner, repo, resp.status_code))
    ...
    with open("./conf/github.yml", "r") as fd:
        ...
        with requests.Session() as session:
            for repo in tqdm.tqdm(repos):
                fork(session, repo[0], token, repo[1])

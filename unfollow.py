...
def unfollow(session, _, token, following):
    _Headers = HEADERS
    ...
    if resp.status_code == 204:
        CustomLogger.info("unfollow people <{0}>".format(following))
    elif resp.status_code == 429:
        raise RateLimitException("api response: 429", 5)
    else:
        CustomLogger.error("failed to unfollow people <{0}>, status code: {1}".format(following, resp.status_code))
    ...
    with open("./conf/github.yml", "r") as fd:
        ...
        with requests.Session() as session:
            for following in tqdm.tqdm(followings):
                unfollow(session, usr, token, following)
